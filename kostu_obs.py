...
def create_table5(weights_file):
    ...
    # Tablo 1'den program çıktılarını ve ilişki değerlerini al
    program_outputs = {}  # {PC1: 0.8, PC2: 0.7, ...}
    for row in range(2, table1_ws.max_row + 1):
        prog_output = table1_ws.cell(row=row, column=1).value
        relation_value = table1_ws.cell(row=row, column=table1_ws.max_column).value
        program_outputs[prog_output] = relation_value

    # Tablo 4'ten her öğrencinin başarı oranlarını al
    current_row = 1
    while current_row < table4_ws.max_row:
        # Öğrenci numarasını bul
        student_header = table4_ws.cell(row=current_row, column=2).value

        # Tip kontrolü ve değer kontrolü ekle
        if not isinstance(student_header, str):
            current_row += 1
            continue

        if not student_header.startswith("Öğrenci"):
            current_row += 1
            continue

        student_no = student_header.split(" ")[1]

        # Başarı oranlarını topla
        success_rates = {}  # {DC1: 95.0, DC2: 98.6, ...}
        row = current_row + 2  # Başlık satırlarını atla
        while row < table4_ws.max_row and table4_ws.cell(row=row, column=1).value:
            dc = table4_ws.cell(row=row, column=1).value
            success_rate = table4_ws.cell(row=row, column=9).value
            if dc and success_rate is not None:
                success_rates[dc] = success_rate
            row += 1

        # Tablo 5'e öğrenci verilerini yaz
        # Başlık
        table5_ws.cell(row=current_row, column=1, value="TABLO 5")
        table5_ws.cell(row=current_row, column=2, value=f"Öğrenci {student_no} için")
        current_row += 1

        # Sütun başlıkları
        table5_ws.cell(row=current_row, column=1, value="Program Çıktıları")
        for col, dc in enumerate(success_rates.keys(), start=2):
            table5_ws.cell(row=current_row, column=col, value=dc)
        table5_ws.cell(row=current_row, column=len(success_rates) + 2, value="Başarı Oranı")
        current_row += 1

        # Program çıktılarının başarı oranlarını hesapla
        for prog_output, relation_value in program_outputs.items():
            # Program çıktısını yaz
            table5_ws.cell(row=current_row, column=1, value=prog_output)

            # Her ders çıktısı için değerleri yaz
            col = 2
            total_success = 0
            count = 0
            for success_rate in success_rates.values():
                if success_rate is not None:
                    table5_ws.cell(row=current_row, column=col, value=round(success_rate, 1))
                    total_success += success_rate
                    count += 1
                col += 1

            # Başarı oranını hesapla ve yaz
            if count > 0 and relation_value > 0:
                average_success = total_success / count
                success_ratio = round(average_success / relation_value, 1)
                table5_ws.cell(row=current_row, column=col, value=success_ratio)

            current_row += 1

        # Sonraki öğrenci için boşluk bırak
        current_row += 2

        # Bir sonraki öğrenciye geç
        while current_row < table4_ws.max_row and not table4_ws.cell(row=current_row, column=2).value:
            current_row += 1
